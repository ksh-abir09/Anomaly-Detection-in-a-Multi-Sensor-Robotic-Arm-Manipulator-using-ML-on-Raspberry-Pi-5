...
# ========= Config =========
# 1) Point to your CSV (or keep relative if the file sits next to this script)
CSV_PATH = r"dataset_final.csv"

# 2) Where to save figures. Change to Desktop if you like:
# OUTDIR = Path.home() / "Desktop" / "Robotic_Hand_Figures"
OUTDIR = Path("figures")
OUTDIR.mkdir(parents=True, exist_ok=True)

# 3) Matplotlib defaults (nice, readable)
plt.rcParams.update({
    "figure.dpi": 150,
    "savefig.dpi": 400,
    "axes.titlesize": 12,
...
def savefig_all(stem: str):
    """Save the current Matplotlib figure as PDF, SVG, and high-DPI PNG."""
    pdf_path = OUTDIR / f"{stem}.pdf"
    svg_path = OUTDIR / f"{stem}.svg"
    png_path = OUTDIR / f"{stem}.png"
    plt.savefig(pdf_path, bbox_inches="tight")           # vector
    plt.savefig(svg_path, bbox_inches="tight")           # vector
    plt.savefig(png_path, bbox_inches="tight", dpi=400)  # hi-res raster
    plt.close()
...
for name, model in models.items():
    ...
    # single-sample latency (batch=1) — increase N for smoother timing
    x1 = X_test[:1]
    _ = model.predict(x1)  # warmup
    N = 500
...
# A) Sensor distributions (Normal vs Fault) — pick representative columns
rep_cols = []
...
corr = pd.DataFrame(X, columns=feature_cols).corr().values
...
plt.tight_layout()
savefig_all("fig_feature_correlation")

# C) Model comparison (Accuracy & F1)
plt.figure(figsize=(8.5, 4.5))
x = np.arange(len(metrics_df)); width = 0.35
plt.bar(x - width/2, metrics_df["accuracy"].values, width, label="Accuracy")
plt.bar(x + width/2, metrics_df["f1"].values, width, label="F1-score")
plt.xticks(x, metrics_df["model"].values, rotation=15)
plt.ylim(0, 1.05); plt.ylabel("Score")
plt.title("Model Comparison")
plt.legend(); plt.grid(axis='y', linewidth=0.3)
for i, v in enumerate(metrics_df["accuracy"].values):
    plt.text(i - width/2, v + 0.02, f"{v:.2f}", ha="center", fontsize=8)
for i, v in enumerate(metrics_df["f1"].values):
    plt.text(i + width/2, v + 0.02, f"{v:.2f}", ha="center", fontsize=8)
plt.tight_layout()
savefig_all("fig_model_comparison")

# D) Latency comparison (batch=1)
plt.figure(figsize=(8.5, 4.5))
plt.bar(metrics_df["model"].values, metrics_df["latency_ms"].values)
plt.ylabel("Latency (ms) @ batch=1")
plt.title("Inference Latency (This Machine)")
...
for (i, j), val in np.ndenumerate(cm):
    plt.text(j, i, f"{val}", ha="center", va="center", fontsize=10)
plt.colorbar(shrink=0.8)
plt.tight_layout()
savefig_all("fig_confusion_matrix")

# F) ROC & PR curves (if probabilities available)
y_prob_best = prob_cache[best_name]
